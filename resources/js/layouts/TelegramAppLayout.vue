...
const rootClasses = computed(() => ['tma-root', props.fill ? 'tma-fill' : '', props.padded ? 'tma-padded' : '', props.class]);
</script>
...
.tma-actions {
    ...
    background: linear-gradient(to top, color-mix(in oklab, var(--color-tg-bg, var(--background)) 100%, transparent), transparent 60%);
    display: flex;
    gap: 0.5rem;
