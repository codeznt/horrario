<template>
    <form @submit.prevent="handleSubmit" class="space-y-6">
        <!-- Service Selection -->
        <FormField id="service_id" :label="t('app.service')" :error="errors.service_id" required>
            <Select v-model="form.service_id" @update:model-value="clearError('service_id')">
                <SelectTrigger>
                    <SelectValue :placeholder="t('app.select_a_service')" />
                </SelectTrigger>
                <SelectContent>
        ...
        <!-- Date Selection -->
        <FormField id="date" :label="t('app.date')" :error="errors.date" required>
        ...
        <!-- Time Slot Selection -->
        <FormField v-if="form.date && availableSlots.length > 0" id="start_time" :label="t('app.time_slot')" :error="errors.start_time" required>
            ...
            <p v-if="availableSlots.length === 0" class="mt-2 text-sm text-tg-hint">
                {{ t('app.no_available_slots_for_date') }}
            </p>
        </FormField>

        <!-- Notes -->
        <FormField id="notes" :label="t('app.notes_optional')" :error="errors.notes" :hint="t('app.special_requests')">
            <Textarea v-model="form.notes" :placeholder="t('app.special_requests')" :rows="3" @input="clearError('notes')" />
        </FormField>

        <!-- Summary Card -->
        <Card v-if="selectedService && form.date && form.start_time" class="bg-tg-section-bg">
            <CardHeader>
                <CardTitle class="text-base">{{ t('app.booking_summary') }}</CardTitle>
            </CardHeader>
            <CardContent class="space-y-2">
                <div class="flex justify-between">
                    <span class="text-tg-subtitle-text">{{ t('app.service') }}:</span>
                    <span class="font-medium">{{ selectedService.title }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-tg-subtitle-text">{{ t('app.date') }}:</span>
                    <span class="font-medium">{{ formatSelectedDate }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-tg-subtitle-text">{{ t('app.time') }}:</span>
                    <span class="font-medium">{{ form.start_time }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-tg-subtitle-text">{{ t('app.duration') }}:</span>
                    <span class="font-medium">{{ selectedService.duration_minutes }} {{ t('app.minutes') }}</span>
                </div>
                <div class="flex justify-between border-t border-tg-section-separator pt-2">
                    <span class="font-medium">{{ t('app.total') }}:</span>
        ...
        <!-- Submit Button -->
        <div class="pt-4">
            <Button type="submit" :disabled="processing || !canSubmit" class="w-full" size="lg">
...
// No emitted events currently; remove defineEmits to satisfy lint
...
const canSubmit = computed(() => {
    return form.value.service_id && form.value.date && form.value.start_time && !processing.value;
});
...
function handleSubmit() {
    const isValid = validate({
        service_id: { required: t('app.please_select_service') },
        date: { required: t('app.please_select_date') },
        start_time: { required: t('app.please_select_time_slot') },
        notes: { maxLength: 500 },
    });
